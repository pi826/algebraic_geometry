...
\Definition{
  ...
  \end{equation*}
  で定義できる新たな$X$上の前層の層化$f^{-1}\mathcal{G}$を\textbf{$\mathcal{G}$の逆像(inverse image of $\mathcal{G}$)}
  という。
}
...
\Proposition{
  $f:Y \to X$を局所環付き空間の閉はめ込みとする。$Z$を局所環付き空間$V(\mathcal{J})$とする。
  ただし、$\mathcal{J} = \ker f^{\#}\subset \mathcal{O}_{X}$.すると$Y\simeq Z$を始域の制限による
  自然な閉はめ込み$f|_{Z}:Z \to X$から得る。
}{
  $f(Y)$は$X$の閉集合であることから
  \begin{equation*}
    (f_{*}\mathcal{O}_{Y})_{x} = \left\{ 
    \begin{alignedat}{2}
      &\quad 0 \qquad &x& \in X \mysetminus f(Y)\\
      &\mathcal{O}_{Y,y} \qquad &x& = f(y)
    \end{alignedat}
    \right.
  \end{equation*}
  を得る。次の完全列とProp:\ref{Prop:1.3.10}から
  \begin{equation*}
    0 \longrightarrow \mathcal{J} \longrightarrow \mathcal{O}_{X} \longrightarrow f_{*}\mathcal{O}_{Y} \longrightarrow 0
  \end{equation*}
  から
  \begin{equation*}
    \mathcal{O}_{X,x}/\mathcal{J}_{x} = (f_{*}\mathcal{O}_{Y})_{x}
  \end{equation*}
  を得る。
  $V(\mathcal{J}) = f(Y)$から
  \begin{equation*}
    X \mysetminus V(\mathcal{J}) = X \mysetminus f(Y)
  \end{equation*}
  なので、$j:Z \hookrightarrow X$を包含写像とすると、$f$から誘導される同相写像$g:Y \to Z$に対して
  \begin{equation*}
    f = j \circ g
  \end{equation*}
  で、
  \begin{equation*}
    j_{*}\mathcal{O}_{Z} = \mathcal{O}_{X}/\mathcal{J} \simeq f_{*}\mathcal{O}_{Y}
  \end{equation*}
  がわかる。容易に
  \begin{equation*}
    f_{*}\mathcal{O}_{Y} = j_{*}g_{*}\mathcal{O}_{Y}
  \end{equation*}
  が分かるので
  \begin{equation*}
    \mathcal{O}_{Z} = j^{-1}j_{*}\mathcal{O}_{Z} \simeq j^{-1}j_{*}g_{*}\mathcal{O}_{Y} = g_{*}\mathcal{O}_{Y}
  \end{equation*}
