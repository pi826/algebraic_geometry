...
\Lemma{
  ...
  \end{equation*}
  とすると，$a,b\notin I^{h}$なのである$i,j$があって，$a_{k}\notin I\cap A_{k},b_{l}\notin I \cap A_{l}$である．
  \begin{equation*}
    ab = \sum_{i = 0}^{n + m}\left( \sum_{j = 0}^{i}a_{j}b_{i - j} \right)
  \end{equation*}
  ただし$a_{n+1}=a_{n+2}=\cdots = 0, b_{m + 1} = b_{m + 2} = \cdots = 0$とする．
  \begin{equation*}
    \sum_{j = 0}^{i} a_{j}b_{i - j} \in I\cap A_{i}
  \end{equation*}
  だから
  よって$ab=a_{n}b_{m} + \{次数n+m未満の斉次元\}$だから$ab$の$n+m$次斉次元は$a_{n}b_{m}$で
  $ab\in I^{h}$より，$a_{n}b_{m} \in I\cap A_{d} \subset I$．また，$I$は素イデアルなので
  \\
  %わからん%
