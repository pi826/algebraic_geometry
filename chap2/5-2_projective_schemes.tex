...
\Lemma{
  ...
  \begin{itemize}
    \item[(1)] 標準的な射$\theta:D_{+}(f) \to \spec{A_{(f)}}$がある．
  ...
  (1)$\proj{A}$は$\spec{A}$の部分集合である．更に，任意の$f\in A$に対して，
  \begin{equation*}
    V(f) \cap \proj{A} = V_{+}(f) = \bigcap_{i}V_{+}(f_{i})
  \end{equation*}
  となる．もし$f = f_{0} + f_{1} + \cdots + f_{d}\ (f_{i} \in A_{i})$ならば
  \begin{equation*}
  ...
  (2)$\theta$が全射であることを示そう．$A_{f}$の次数$n\ (n\in \mathbf{Z})$の斉次元は$af^{-N}\ (a\in A, \deg{a} = Nr+n)$の形になる．
  $\mathfrak{q}\in \spec{A_{(f)}}$を取る．$A_{f}$は次数$A_{(f)}$代数であることに注意すれば，
