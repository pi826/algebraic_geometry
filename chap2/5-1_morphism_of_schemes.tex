...
\Example{
  $X$をスキームとする．$x\in X$に対して$k(x)$は点$x$での$X$の剰余体であった．(Def:\ref{Def:1.4.1}を参照．)このとき標準的な全射$\mathcal{O}_{X,x} \to k(x) = \mathcal{O}_{X,x}/\mathfrak{m}_{x}$
  は閉はめ込み$\spec{k(x)} \to \spec{\mathcal{O}_{X,x}}$を誘導する．Ex:\ref{Ex:1.5.9}より
  射$\mathcal{O}_{X,x} \to X$がある．よって射$\spec{k(x)} \to X$を得る．この射は$\spec{k(x)}$の
