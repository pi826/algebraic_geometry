...
\Lemma{
  ...
  みたように，標準的な全射
  \begin{equation*}
    \mathcal{O}_{\spec{A}}(D(f)) = A_{f} \to (A/I)_{\varphi(f)} = i_{*}\mathcal{O}_{\spec{A}}(D(f))
  \end{equation*}
  がある．これにより，$i^{\#}$の全射性と，$(\ker{i^{\#}})(D(f)) = I_{f} = I\otimes_{A}A_{f}$
  がわかる．
}

\Definition{
  $Z$を$X$の閉集合とする．このとき$Z$が\textbf{閉部分スキーム(closed subscheme)}とは
  包含写像$j:Z\to X$が閉はめ込み
  \begin{equation*}
...
\Definition{
  $S$をスキームとする．このとき$X$が\textbf{$S$-スキーム($S$-scheme)}または\textbf{$S$上のスキーム(scheme over $S$)}とは
  スキームの射$\pi:X \to S$が与えられているときをいう．この$\pi$を\textbf{構造射(structural morphism,\ structure morphism)}
  ，$S$を\textbf{基底スキーム(base scheme)}という．
}
